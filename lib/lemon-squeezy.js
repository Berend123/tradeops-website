...
export async function createLemonCheckoutSession({
  ...
  const affiliateTrackingId = clean(payload.affiliate_tracking_id);
  const attributedCheckoutUrl = affiliateTrackingId
    ? appendQueryParam(checkoutUrl, "aff_ref", affiliateTrackingId, origin)
    : checkoutUrl;

  return {
    session_id: clean(data.id) || crypto.createHash("sha256").update(checkoutUrl, "utf8").digest("hex").slice(0, 16),
    provider: "lemon_squeezy",
    checkout_url: attributedCheckoutUrl,
